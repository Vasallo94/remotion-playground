...
- **Catálogo y logs Pi más seguros** — `generate_remotion_config` deja de inventar terminales `/compact`, falla con error accionable si una escena terminal no trae `lines`, preserva escenas `custom` registradas por `componentId`, devuelve resumen accionable del catálogo y expone eventos Pi básicos en el log lateral.
