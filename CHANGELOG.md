...
- **Minimal Visual Recipe artifacts and production lineage** — reuses the existing artifact store for deterministic compiled recipes, event-boundary evidence, target-scoped active sets, tamper verification, immutable config projection, and exact QA/validation/render/review/publication lineage without new tables, source writes, or mutable registries.
...
- **Harness architecture swarm evidence** — isolated Sol/high architecture/security reviews, Luna/high refactoring/simplification/readiness reviews, and a Sol/high integration report establish four bounded implementation slices before the next browser-operated novel-scene E2E.
- **Explicit failed-action retry** — adds a single-use parent authorization and `/api/pi/retry` boundary that retries only the exact current failed journal key after restart, increments durable generation before effects, rejects ordinary/duplicate/stale/checkpoint-blocked retries, and wires the existing web error banner.
- **Journaled Scene QA revision cycle** — rejected visual findings now create an exact hash-bound direction revision request, run a distinct parent-owned action, require CP2 again, invalidate stale config/QA through lineage, and rerender before progression; ADR 0044.
- **Explicit tutorial watermark control** — adds a backward-compatible `watermark` config field so approved brand-free renders can disable the composition-level logo and be verified by Scene QA; ADR 0043.
- **Política de prompts agnósticos a la temática** — ADR 0022 separa responsabilidades estables de cada agente frente a tema, marca, audiencia, formato e idioma como inputs; `agent-pi` deja de cargar en bloque los prompts legacy de DeepAgents y cada especialista Pi usa un prompt curado sin recetas por categoría.
- **Legacy main AgentSession eliminado** — borra construcción SDK, ResourceLoader, tools, event bridge y disposal de la sesión orquestadora; solo sobreviven sesiones aisladas de especialistas con ownership por acción.
- **Plan creation y mode routing parent-owned** — nuevas threads exigen PipelineMode explícito, parent persiste plan canónico antes de intake, mode es inmutable y cualquier acción sin adapter falla sin prompt/model/tool fallback; web envía new_video explícito; ADR 0042.
- **Render review, checkpoint final y publicación parent-owned** — review determinista guardada como proposal, checkpoint artifact/version-bound, publicación exige aprobación humana passing, escribe conjunto exacto y verifica SHA-256; reanuda crash por escritura idempotente y deduplica por journal. Tools retirados del modelo principal.
- **Render parent-owned con idempotencia de provider** — render-service persiste Idempotency-Key+request hash únicos, reutiliza jobs idénticos y rechaza key con input cambiado; parent reanuda attempts in-progress, consulta el mismo job y guarda render_job+plan+success atómicamente; ADR 0041.
- **Validación final conectada al executor** — valida el config exacto tras audio, persiste validation_report con config hash/lineage, completa plan y action atómicamente, y deduplica por journal. Tool retirado del modelo principal.
- **Producción silent/local de audio conectada exactly-once** — ejecución parent-owned, manifest+plan+action atómicos, duplicados journaled no repiten producción y música local es determinista. Voice API falla antes del claim hasta disponer de receipt/provider-key durable.
- **Audio planning y CP3 conectados al executor** — preferencias derivadas del brief, exactamente un target, validación parent-owned contra voces/librería/escenas y commit atómico de audio chart + plan + action; CP3 ligado a artifact/version. Tools retirados del modelo principal.
- **Scene QA y checkpoint visual conectados al executor** — stills renderizados por el parent, exactamente un target pasado al reviewer, QA proposal guardada con plan/action atómicos y checkpoint solo para findings no-PASS. Tools retirados del modelo principal.
- **Scene Composer y CP4 conectados al executor** — recibe exactamente un target, resuelve reuse/composed/gap, valida script revisado y guarda composition+script+markdown+checkpoint+plan+action atómicamente. Capability gap presenta CP4 directamente sin habilitar código ni promoción.
- **Research/skip conectado al executor parent-owned** — la decisión sale exclusivamente del production brief aprobado; skip aplica un override canónico limitado y run persiste research citado aprobado + plan + action success atómicamente. Tool retirado del modelo principal.
- **Copywriter/direction y CP1/CP2 conectados al executor** — brief creativo derivado del production brief, evidence research parent-owned, exactamente un selected target, proposals sin persistencia interna, validación de catálogo y commits atómicos de script+markdown/direction/checkpoint/plan/action. Tools equivalentes retirados del modelo principal.
- **Config draft/final conectado al executor parent-owned** — bloquea el tool libre del modelo, construye input desde brief/target/script/direction/audio aprobados y guarda config + lineage + plan effects + action success atómicamente; final config exige y liga draft previo/audio. Corrige idempotencia falsa por step compartido.
- **Target resolution conectado al executor parent-owned** — `resolve_target` exige run_intake journaled, liga el brief vigente y guarda atómicamente selected target o target-clarification checkpoint con action success; revisiones humanas generan una nueva clave por versión del brief.
- **Intake conectado al executor parent-owned** — `run_intake` ya reclama journal antes del especialista y guarda production brief + clarification checkpoint + action success en una transacción; startup convierte claims internos interrumpidos en failure con retry explícito.
- **Reconciliación por clase de efecto** — cubre todas las acciones canónicas con atomic artifact/checkpoint commit, provider-idempotency query, content-hash verification o no-effect; añade commit SQLite conjunto de artifacts+action y rollback ante generation stale; ADR 0040.
- **Executor durable de acciones parent-owned** — evalúa requests canónicos, fingerprint exacto, claim previo al efecto, completion/failure ligado a generation, duplicados restart-safe y retry explícito sin conocer especialistas/render/publicación/promoción; ADR 0039.
- **Promoción Tier 2 durable, todavía desconectada** — sustituye autoridad process-local por journal SQLite versionado con evidence/plan/checkpoint/approval/preimages/progreso/rollback persistentes, claims `BEGIN IMMEDIATE`, recuperación restart de staging/commits mixtos/all-after, rollback durable y `manual_intervention` ante drift desconocido; ADR 0038 Accepted. Session/tools siguen sin poder promover.
- **Verificación Tier 2 unificada y fail-closed** — consume el manifest autenticado de candidatePolicy sin schema paralelo, sella source/harness, revalida policy antes/después de format/typecheck/lint/bundle/stills, usa procesos sin shell y bloquea tampering, symlinks, stale outputs, timeouts, output caps y still evidence inválida; ADR 0037.
- **Política estática Tier 2 para candidatos ejecutables** — manifest versionado ligado a CP4, límites/hash/destino/dependencias, análisis AST sin ejecutar código y rechazo adversarial de imports/runtime/red/proceso/eval/globals/nondeterminismo/HTML-style-URLs/CSS motion/assets inseguros y bypasses por alias; ADR 0036.
- **Matriz de recuperación y checkpoints endurecidos** — valida decisión booleana, checkpoint/artifact/version/thread/kind/latest antes de mutar, conserva compatibilidad con decisiones legacy, añade fixtures CP1–CP4/QA/audio/final/intake/target, restart SQLite, replay SSE y exactamente-una-vez mockeado.
- **Configurador estricto ligado al target** — construye input exclusivamente desde brief/target/script/direction/audio aprobados, valida capacidades y lineage, bloquea audio no aprobado, rechaza config stale, persiste hash/lineage separado y conserva un único repair turn; ADR 0035.
- **Intake y target parent-owned integrados** — `new_video` ejecuta intake tipado y resolución explícita de destino antes de research, persiste `production_brief`/`selected_target`, presenta aclaraciones recuperables, vincula decisiones a checkpoint/artifact/version y rechaza targets ambiguos, contradictorios o stale sin recurrir a defaults ni al main model.
- **Journal durable de acciones parent-owned** — añade migración SQLite versionada y transacciones `BEGIN IMMEDIATE` para claims started/succeeded/failed, fingerprints canónicos, conflictos de identidad, retry explícito, generaciones contra callbacks stale, validación fail-closed de filas/migraciones y recuperación tras restart; ADR 0034.
- **Fundamento de ejecución directa determinista** — centraliza modos, pasos y transiciones canónicas profundamente inmutables; añade handlers parent-owned puros con prerequisitos, artifacts aceptados, idempotency keys, efectos y rechazos por stale/unapproved/invalid plan; reserva intake/target antes de research sin cambiar todavía la autoridad de la sesión activa.
- **Contratos abstractos de destino** — añade `TargetContract` versionado, registry parent-owned profundamente inmutable, resolución exclusivamente desde selectores técnicos explícitos, validación estricta de capacidades/escenas/render/publicación y resúmenes de especialista solo después de seleccionar un destino exacto.
- **Intake de producción tipado y neutral** — añade `ProductionBrief` con estados provided/explicitly-absent/unresolved, preguntas humanas enfocadas, especialista aislado con una única salida estructurada y parent ownership de versión, investigación, unresolved fields y repair; no introduce defaults de composición, formato, dimensiones, tema, plataforma o idioma.
- **Integridad del workspace y cleanup seguro** — añade manifests SHA-256 pre/post para código, specs y configuración, comparación con drift de Git, guardas registradas para cleanup recursivo de tests, pruebas contra traversal/root/symlinks/prefijos incorrectos y un ADR específico; la suite completa y el bundle no alteran el manifest.
- **Guard de neutralidad para prompts Pi** — elimina identificadores de composición, formatos, dimensiones, temas, idiomas por defecto y recetas temáticas del prompt principal y de todos los especialistas; un test recorre cada prompt y bloquea su reintroducción.
- **Configurador aislado y abstracto** — añade un especialista estructurado que compila artifacts aprobados contra el contrato de destino suministrado, valida mediante el parent/render-service y permite un único repair turn sin asumir tipo de vídeo, composición, formato, tema o idioma.
- **Coordinador Pi canónico y gate de retirada de LangGraph** — planes por modo ignoran pasos inventados y rechazan updates/decisiones desconocidas; `new_video` deriva la siguiente transición desde SQLite/artifacts/checkpoints y repara una parada prematura con una instrucción acotada; el main pierde tools de draft/save/decisión directas, web y Compose pasan a Pi por defecto y se añade Dockerfile de `agent-pi`; ADR 0029 define que LangGraph solo se elimina tras E2E y paridad de todos los modos.
- **Contratos exactos de props custom** — el generador de catálogo extrae automáticamente interfaces/types TypeScript de cada componente y las entrega a copywriter/compositor para evitar props plausibles pero inexistentes; la prueba live corrigió `timeline` y `bar-chart` tras detectar shapes inválidos.
- **Creación de escenas declarativa-first** — nuevo workspace `@claqueta/scene-contracts` como contrato único y estricto, runtime Remotion `composed-scene` con primitivas semánticas/theme/movimiento frame-based, validación previa en agent-pi y especialista aislado `run_scene_composer_specialist` sin filesystem/shell/código; las necesidades no expresables producen capability gaps reusables y CP4 antes de cualquier escalado ejecutable; ADR 0028 define cuarentena, verificación visual y promoción atómica para la siguiente capa.
- **Revisión determinista y aceptación final del MP4** — el render-service revisa jobs completados con `ffprobe` sin shell y reporta tamaño, duración/tolerancia, vídeo, dimensiones, FPS, codec y audio esperado; Pi persiste `render-review.json`, actualiza `review` y presenta un checkpoint humano final antes de publicar; ADR 0027 documenta la separación entre checks técnicos y aceptación estética.
- **Scene QA visual Pi multimodal** — `run_scene_qa_specialist` valida manifests/rutas/tamaños de PNG del render-service, adjunta stills ordenados a una sesión Pi aislada y topic-neutral, exige cobertura estructurada por escena, persiste `qa-report.json` y presenta checkpoint humano ante cualquier cambio sin autoaplicar sugerencias; el endpoint de stills sanea ids y elimina ejecución con shell; ADR 0026 documenta el límite de autoridad.
- **Producción determinista de audio aprobado** — `produce_approved_audio_assets` consume CP3/config sin otro LLM, omite capas silenciosas, copia música local sin credenciales, bloquea TTS salvo `CLAQUETA_PI_ALLOW_AUDIO_GENERATION=true`, ejecuta el generador TypeScript sin shell y verifica/persiste un manifest antes del render; ADR 0025 documenta la decisión.
- **Cuarto especialista Pi aislado: audio planner** — `run_audio_planner_specialist` planifica silencio, voz Gemini single/multi-speaker y music bed exclusivamente desde inventario local, valida voces/índices/volúmenes/speakers/library ids, persiste `audio-chart.json`, presenta CP3, incorpora automáticamente la carta aprobada al config y mantiene generación/copia de assets fuera del especialista; ADR 0024 documenta la separación entre criterio creativo y producción.
- **Tercer especialista Pi aislado: researcher** — `run_research_specialist` usa una sesión Pi sin tools generales con búsqueda DuckDuckGo y fetch HTTPS público capados, salida `ResearchBrief` con claims/citas/confianza, artifact `brief.json`, plan/model route/telemetría y consumo automático por el copywriter; ADR 0023 documenta provenance, ejecución condicional y guardas SSRF para DNS, IPs privadas, redirects, timeout, tamaño y presupuesto de llamadas.
- **Segundo especialista Pi aislado: copywriter** — `run_copywriter_specialist` convierte petición + brief explícito en un `ScriptDraft` estructurado mediante una sesión Pi sin tools generales, valida el resultado contra el catálogo antes de persistirlo, soporta feedback de CP1 y emite telemetría/plan/model route; el smoke real con una microhistoria epistolar produjo 6 escenas variadas sin recetas tecnológicas.
- **Primer especialista Pi aislado: director** — `run_direction_specialist` crea una sesión Pi SDK interna sin tools generales, exige salida estructurada mediante `submit_direction`, valida que no cambie ids/tipos/componentes aprobados, soporta revisiones con feedback de CP2, persiste el artifact, registra model route/estado en el plan y emite lifecycle SSE replayable que el frontend adapta a `SubagentCard`; incluye smoke test real agnóstico al tema y ADR 0021.
- **Fallback de modelo Pi compatible con cuentas ChatGPT** — el router evita seleccionar automáticamente variantes Codex Spark que aparecen disponibles en el registry pero no funcionan con cuentas Codex respaldadas por ChatGPT; las rutas explícitas por env mantienen prioridad.
- **Draft de vídeo sobre algoritmos de optimización** — nuevo contenido generado por `agent-pi` en `content/tutorials/optimizacion-no-es-probarlo-todo/` con `script.json`, `script.md`, `direction.json` y `config.json`; queda como evidencia de QA editorial para mejorar el runtime antes de considerar el resultado final.
...
- **Plan de pipeline persistente en `agent-pi`** — nueva tabla SQLite `pipeline_plans`, tools `create_pipeline_plan`/`read_pipeline_plan`/`update_pipeline_step`/`record_pipeline_decision`/`get_next_pipeline_step`, SSE `plan_updated`, snapshot `/api/pi/thread/:threadId` con plan y `usePiVideoStream` rellenando `planState` desde el snapshot o los eventos para que la UI recupere el estado real del pipeline tras reload.
- **Catálogo, dirección y logs Pi más seguros** — `create_script_draft` usa schema estricto para `type`/`visualType`, pide `visualRole`/`propsPlan` para separar rol visual humano de contrato renderizable y devuelve feedback recuperable cuando detecta tipos/visualTypes/componentIds inventados antes de mostrar escaletas, `generate_remotion_config` rechaza tipos desconocidos en vez de degradarlos a `callout`, deja de inventar terminales `/compact`, falla con error accionable si una escena terminal no trae `lines`, preserva escenas `custom` registradas por `componentId`, devuelve resumen accionable del catálogo, fuerza render síncrono salvo opt-in explícito, recupera el video renderizado desde snapshots de thread y mejora las cards de guion/dirección para mostrar contratos visuales reales.
...
- **Planificación visual por escena en el checkpoint de guion** — `ScriptDraft` y el checkpoint de guion ahora incluyen `narrativeRole`, `visualType`, `componentId`, `visualRationale`, `requiredAssets`, `missingCapabilities` y `riskNotes`; `scriptToMarkdown()` y `ScriptCard` muestran y editan esos campos sin romper guiones antiguos, y el prompt Pi exige justificar la elección visual escena por escena sin recetas hardcodeadas por tipo de vídeo.
