...
- **Catálogo, dirección y logs Pi más seguros** — `create_script_draft` y `generate_remotion_config` rechazan tipos/visualTypes/componentIds inventados antes de mostrar escaletas o degradar escenas a `callout`, `generate_remotion_config` deja de inventar terminales `/compact`, falla con error accionable si una escena terminal no trae `lines`, preserva escenas `custom` registradas por `componentId`, devuelve resumen accionable del catálogo, fuerza render síncrono salvo opt-in explícito, recupera el video renderizado desde snapshots de thread y mejora las cards de guion/dirección para mostrar contratos visuales reales.
