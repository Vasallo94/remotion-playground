...
- **Catálogo, dirección y logs Pi más seguros** — `generate_remotion_config` deja de inventar terminales `/compact`, falla con error accionable si una escena terminal no trae `lines`, rechaza tipos de escena desconocidos en vez de degradarlos a `callout`, preserva escenas `custom` registradas por `componentId`, devuelve resumen accionable del catálogo, fuerza render síncrono salvo opt-in explícito, recupera el video renderizado desde snapshots de thread y mejora la card de dirección para mostrar visuales reales.
