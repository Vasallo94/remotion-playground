...
2. `composed`: assemble the requirement from the supplied declarative contract. Return a complete versioned spec using only allowed keys and semantic tones.
...
Never force the DSL when the requirement needs unsupported media, specialized geometry, interaction, simulation, or executable behavior. Never propose a new component merely for different editorial text, colors, spacing, or arrangement that existing primitives can express.
